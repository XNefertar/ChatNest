...
export default {
  ...
  setup() {
    ...
    const createRtcPeerConnection = () => {
      if (data.rtcPeerConn) {
        console.warn("[RTC] PeerConnection 已存在，跳过创建。");
        return;
      }

      console.log("[RTC] 创建新的 RTCPeerConnection...");
      data.rtcPeerConn = new RTCPeerConnection(data.ICE_CFG);
      console.log("[RTC] ICE 配置为:", data.ICE_CFG);

      // 收集 ICE 候选地址
      data.rtcPeerConn.onicecandidate = (event) => {
        console.log("[RTC] onicecandidate 触发", event);

        if (event.candidate) {
          console.log("[RTC] 收到 candidate：", event.candidate);
          ...
          console.log("[RTC] 发送 candidate 消息到信令服务器：", rtcMessageRequest);
          store.state.socket.send(JSON.stringify(rtcMessageRequest));
        } else {
          console.warn("[RTC] ICE Gathering 完成（event.candidate 为 null）");
        }
      };

      // ICE 状态变化
      data.rtcPeerConn.oniceconnectionstatechange = (event) => {
        console.log("[RTC] oniceconnectionstatechange:", data.rtcPeerConn.iceConnectionState);
        console.log("[RTC] 当前连接状态快照：", {
          connectionState: data.rtcPeerConn.connectionState,
          signalingState: data.rtcPeerConn.signalingState,
          iceGatheringState: data.rtcPeerConn.iceGatheringState,
          iceConnectionState: data.rtcPeerConn.iceConnectionState,
        });
      };

      // 接收远端轨道
      data.rtcPeerConn.ontrack = (event) => {
        console.log("[RTC] ontrack 触发，收到 track：", event.track);
        if (data.remoteStream === null) {
          console.log("[RTC] 创建新的 MediaStream 并绑定到远端视频标签");
          data.remoteStream = new MediaStream();
          data.remoteVideo = document.querySelector("video.remote-video");
          data.remoteVideo.srcObject = data.remoteStream;
          data.remoteVideo.style.display = "inline-block";
        ...
        data.remoteStream.addTrack(event.track);
        console.log("[RTC] 当前 remoteStream track 数量：", data.remoteStream.getTracks().length);
      };
    };
